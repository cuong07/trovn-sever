  ...
  try {
    ...
    if (user.isLooked) {
      return res
        ...
        .json(BaseResponse.error("Tài khoản của bạn đã bị vô hiệu hóa", null));
    }
    req.user = user;
    next();
  } catch (error) {
...
export const verifyTokenOptional = async (req, res, next) => {
  ...
  try {
    ...
    if (error.name === "TokenExpiredError") {
      logger.warn("Token hết hạn, tiếp tục mà không có user.");
    } else {
      logger.warn("Token không hợp lệ, tiếp tục mà không có user.");
    }
  }
  next();
};
