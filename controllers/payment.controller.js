...
const PaymentController = {
  ...
  async createMomoPayment(req, res) {
    const { amount, orderInfo, buyPackageId, requestType, packageType  } = req.query;
    console.log(amount, orderInfo, buyPackageId, requestType, packageType );
    ...
    try {
      ...
      const requestBody = JSON.stringify({
        ...
        extraData: packageType + "," + buyPackageId,
  ...
  async createZaloPayPayment(req, res) {
    const { amount, orderInfo, buyPackageId } = req.query;
    const { id } = req.user;
  ...
  async createVNPayPayment(req, res) {
    let { amount, locale, orderInfo, bankCode, buyPackageId } = req.query;
    const { id } = req.user;
    ...
    vnp_Params["vnp_OrderInfo"] = buyPackageId;
  ...
  async callbackVNPayIPN(req, res) {
    try {
      ...
      const payment = await PaymentService.getPaymentByTransactionId(orderId);
      const { user } = await PaymentService.getUserForTransactionId(orderId);
          ...
          if (checkAmount) {
            if (!paymentStatus) {
              ...
              if (rspCode == "00") {
                ...
                logger.info(subject);
                res.status(statusCode.OK).json(
                  BaseResponse.success("Giao dịch không thành công", {
                    code: "00",
                    payment,
                  })
                );
              ...
              subject = "Đơn hàng này đã được cập nhật trạng thái thanh toán";
              logger.info(subject);
              res
                .status(statusCode.OK)
                .json(
